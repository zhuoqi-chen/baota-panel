...
function bindBTPanel(a,type,ip,btid,url,user,pw){
	var titleName = "关联宝塔面板";
	if(type == "b"){
		btn = "<button type='button' class='btn btn-success btn-sm' onclick=\"bindBTPanel(1,'b')\">添加</button>";
	}
	else{
		titleName = "修改关联"+ip;
		btn = "<button type='button' class='btn btn-default btn-sm' onclick=\"bindBTPaneldel('"+btid+"')\">删除</button><button type='button' class='btn btn-success btn-sm' onclick=\"bindBTPanel(1,'c','"+ip+"','"+btid+"')\" style='margin-left:7px'>修改</button>";
	}
	...
	layer.open({
		...
		<div class='info-r'><input class='bt-input-text' type='text' name='btaddress' id='btaddress' value='"+url+"' placeholder='面板登录地址' style='width:100%'/></div>\
		</div>\
		...
		<div class='info-r'><input class='bt-input-text' type='text' name='bttitle' id='bttitle' value='"+ip+"' placeholder='面板备注' style='width:100%'/></div>\
		</div>\
		<div class='line'><ul class='help-info-text c7'><li>收藏其它服务器面板资料，实现一键登录面板功能</li><li>面板备注不可重复</li></ul></div>\
		<div class='bt-form-submit-btn'><button type='button' class='btn btn-danger btn-sm' onclick=\"layer.closeAll()\">取消</button> "+btn+"</div></div>"
	});
