...
function GetFiles(Path) {
	...
	$.post('/files?action=GetDir', data, function(rdata) {
		...
		BarTools += ' <button onclick="javascript:GetFiles(\'' + rdata.PATH + '\');" class="btn btn-default btn-sm glyphicon glyphicon-refresh" title="刷新"></button> <button onclick="ExecShell()" title="终端" type="button" class="btn btn-default btn-sm"><em class="ico-cmd"></em></button>';
...
function GetDisk() {
	...
	$.get('/system?action=GetDiskInfo', function(rdata) {
		...
		var trash = '<span id="recycle_bin" onclick="Recycle_bin(\'open\')" title="收回站" style="position: absolute; border-color: #ccc; right: 77px;"><span class="glyphicon glyphicon-trash"></span>&nbsp;回收站</span>';
...
function DeleteFile(fileName){
	layer.confirm("您确实要把此文件["+fileName+"]放入回收站吗?",{title:'删除文件',closeBtn:2,icon:3},function(){
...
function DeleteDir(dirName){
	layer.confirm("您确实要把此目录["+dirName+"]放入回收站吗?",{title:'删除目录',closeBtn:2,icon:3},function(){
		layer.msg('正在执行,请稍候...',{icon:16,time:0,shade: [0.3, '#000']});
...
function ExecShell(action){
	...
	layer.open({
		...
		closeBtn: 2,
		area: ['70%','600px'], //宽高
		title: '执行SHELL (仅支持非交互命令)',
		content: '<div class="bt-form pd15">\
					<div class="shellcode"><pre id="Result"></pre></div>\
					<div class="line">\
					<input type="text" class="bt-input-text" name="exec" id="mExec" value="" placeholder="SHELL命令" onkeydown="if(event.keyCode==13)ExecShell(1);" /><span class="shellbutton btn btn-default btn-sm pull-right" onclick="ExecShell(1)" style="width:10%">发送</span>\
					</div>\
				</div>'
	});
	setTimeout(function(){
