...
function phpSoftMain(name,key){
	...
	$.get('/plugin?action=getPluginInfo&name=php',function(rdata){
		...
		bodys = [
				...
				'<p data-id="4"><a id="phpext" href="javascript:SetPHPConfig(\''+name+'\','+nameA.pathinfo+')">安装扩展</a><span class="spanmove"></span></p>',
...
function service(name,status){
	...
	var help = '<ul class="help-info-text c7 mtb15" style="padding-top:30px"><li>机器内存小于1G，不建议使用mysql5.5以上版本</li><li>如果数据库经常自动停止，请尝试使用linux工具箱增加SWAP或者升级服务器内存</li></ul>';
	if(name == 'mysqld'){
...
function SetPHPConfig(version,pathinfo,go){
	$.get('/ajax?action=GetPHPConfig&version='+version,function(rdata){
		...
		var pathinfo1 = '<tr id="pathInfo"><td>PATH_INFO</td><td>扩展配置</td><td>MVC架构的程序需要开启,如typecho</td><td><span class="ico-'+(rdata.pathinfo?'start':'stop')+' glyphicon glyphicon-'+(rdata.pathinfo?'ok':'remove')+'"></span></td><td style="text-align: right;" width="50">'+pathinfoOpt+'</td></tr>';
...
function SetWafConfig(name,value){
	if(name == 'CCrate'){
		...
		if (CCrate_1 < 1 || CCrate_1 > 3000 || CCrate_2 < 1 || CCrate_2 > 1800){
			layer.msg("CC防御配置值超出可用值 (频率1-3000|周期1-1800)",{icon:5});
			return;
		}
...
function phpVer(name,version){
	$.post('/site?action=GetPHPVersion',function(rdata){
		var body = "<div class='ver line'><span class='tname'>PHP版本</span><select id='get' class='bt-input-text mr20' name='phpVersion' style='width:110px'>";
		var optionSelect = '';
