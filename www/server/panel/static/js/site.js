...
function limitNet(id){
	$.post('site?action=GetLimitNet&id='+id,function(rdata){
		...
		var limitList = "<option value='1' "+((rdata.perserver == 0 || rdata.perserver == 300)?'selected':'')+">论坛/博客</option>"
						+"<option value='2' "+((rdata.perserver == 200)?'selected':'')+">图片站</option>"
						+"<option value='3' "+((rdata.perserver == 50)?'selected':'')+">下载站</option>"
						+"<option value='4' "+((rdata.perserver == 500)?'selected':'')+">商城</option>"
						+"<option value='5'  "+((rdata.perserver == 400)?'selected':'')+">门户</option>"
						+"<option value='6' "+((rdata.perserver == 60)?'selected':'')+">企业站</option>"
						+"<option value='7' "+((rdata.perserver == 150)?'selected':'')+">视频站</option>"
...
function Proxy(siteName,type){
	...
	$.post('/site?action=GetProxy','name='+siteName,function(rdata){
		...
		var body = "<div>"
			   +"<p style='margin-bottom:8px'><span>目标URL</span><input "+disabled+" class='bt-input-text' type='text' name='toUrl' value='"+rdata.proxyUrl+"' style='margin-left: 5px;width: 380px;height: 30px;margin-right:10px;' placeholder='请填写完整URL,例：http://www.test.com' /></p>"
			   +"<p style='margin-bottom:8px'><span>发送域名</span><input "+disabled+" class='bt-input-text' type='text' name='toDomain' value='"+rdata.toDomain+"' style='margin-left: 5px;width: 380px;height: 30px;margin-right:10px;' placeholder='发送到目标服务器的域名,例：www.test.com' /></p>"
			   +"<p style='margin-bottom:8px'><span>内容替换</span><input "+disabled+" class='bt-input-text' type='text' name='sub1' value='"+rdata.sub1+"' style='margin-left: 5px;width: 182px;height: 30px;margin-right:10px;' placeholder='被替换的文本,可留空' />"
			   +"<input class='bt-input-text' type='text' name='sub2' "+disabled+" value='"+rdata.sub2+"' style='margin-left: 5px;width: 183px;height: 30px;margin-right:10px;' placeholder='替换为,可留空' /></p>"
...
function To301(siteName,type){
	...
	$.post('/site?action=Get301Status','siteName='+siteName,function(rdata){
		...
		var body = "<div>"
			   +"<p style='margin-bottom:8px'><span>访问域名</span><select class='bt-input-text' name='srcDomain' style='margin-left: 5px;width: 380px;height: 30px;margin-right:10px;"+(rdata.status?'background-color: #eee;':'')+"' "+(rdata.status?'disabled':'')+"><option value='all'>整站</option>"+domains+"</select></p>"
			   +"<p style='margin-bottom:8px'><span>目标URL</span><input class='bt-input-text' type='text' name='toUrl' value='"+rdata.url+"' style='margin-left: 5px;width: 380px;height: 30px;margin-right:10px;"+(rdata.status?'background-color: #eee;':'')+"' placeholder='请填写完整URL,例：http://www.test.com' "+isRead+" /></p>"
	...
	$.post('site?action=GetSSL','siteName='+siteName,function(rdata){
		switch(rdata.type){
			case -1:
				...
				var txt = "<div class='mtb15'>本站点未设置SSL，如需设置SSL，请选择切换类目申请开启SSL<br><p style='color:red;'>关闭SSL以后,请务必清除浏览器缓存再访问站点</p></div>";
				$(".tab-con").html(txt);
				break;
			case 1:
...
function closeSSL(siteName){
	$.post('site?action=GetSSL','siteName='+siteName,function(rdata){
		switch(rdata.type){
			case -1:
				var txt = "<div class='mtb15'>本站点未设置SSL，如需设置SSL，请选择切换类目申请开启SSL<br>关闭SSL以后,请务必清除浏览器缓存再访问站点<p style='color:red;'>关闭SSL以后,请务必清除浏览器缓存再访问站点</p></div>";
				setCookie('letssl',0);
				$(".tab-con").html(txt);
				break;
			case 1:
...
function BTssl(type,id,siteName){
	...
	var b = '<div class="btssl"><div class="line mtb15"><span class="tname text-center">域名</span><select id="domainlist" class="bt-input-text" style="width:220px"></select></div>'
		  ...
		  + '<div class="btssllist mtb15" style="height:171px;overflow:auto"><div class="divtable"><table class="table table-hover"><thead><tr><th>域名</th><th>到期时间</th><th>状态</th><th class="text-right" width="100">操作</th></tr></thead><tbody id="ssllist"></tbody></table></div></div>'
	...
	var lets =  '<div class="btssl"><div class="line mtb15"><span class="tname text-center">域名</span><ul id="ymlist" style="padding: 5px 10px;max-height:200px;overflow:auto; width:240px;border:#ccc 1px solid;border-radius:3px"></ul></div>'
			  ...
			  + '<ul class="help-info-text c7 ptb15"><li>申请之前，请确保域名已解析，如未解析会导致审核失败。</li><li>本证书申请失败率较高，如多次失败请使用其他SSL</li><li>Let\'s Encrypt免费证书，有效期3个月，支持多域名。默认会自动续签</li><li>若您的站点使用了CDN或301重定向会导致续签失败</li></ul>'
			  + '</div>';
	...
	switch(type){
			...
			if(getCookie('letssl') == 1){
				$.post('site?action=GetSSL','siteName='+siteName,function(rdata){
					var lets = '<div class="myKeyCon ptb15"><div class="ssl-con-key pull-left mr20">密钥(KEY)<br><textarea id="key" class="bt-input-text" readonly="" style="background-color:#f6f6f6">'+rdata.key+'</textarea></div>'
						...
						+ '<ul class="help-info-text c7 pull-left"><li>已为您自动生成Let\'s Encrypt免费证书；</li><li>如需使用其他SSL,请切换其他证书后粘贴您的KEY以及CRT内容，然后保存即可。</li></ul>';
					$(".tab-con").html(lets);
					$(".help-info-text").after("<div class='line mtb15'><button class='btn btn-default btn-sm' onclick='OcSSL(\'CloseSSLConf\',\'"+siteName+"\')' style='margin-left:10px'>关闭SSL</button></div>");
				});
				return;
			}
			$(".tab-con").html(lets);
		...
		case 'other':
			...
			$.post('site?action=GetSSL','siteName='+siteName,function(rdata){
				if(rdata.status){
					$(".ssl-btn").append("<button class='btn btn-default btn-sm' onclick=\"OcSSL('CloseSSLConf','"+siteName+"')\" style='margin-left:10px'>关闭SSL</button>");
				}
		...
		if(rdata.status){
			for(var i=0;i<rdata.data.length;i++){
				...
				if(rdata.data[i].setup){
					txt = '已部署 | <a class="btlink" href="javascript:OcSSL(\'CloseSSLConf\',\''+siteName+'\')">关闭</a></div>';
				}
